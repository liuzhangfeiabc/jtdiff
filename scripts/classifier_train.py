...
def main():
    ...
    def forward_backward_log(data_loader, prefix='train'):
        ...
        if args.noised:
            ...
            t = jt.zeros(batch.shape[0], dtype=jt.long, device=dist_util.dev())
...
def save_model(mp_trainer, opt, step):
    if (dist.get_rank() == 0):
        jt.save(mp_trainer.master_params_to_state_dict(mp_trainer.master_params), os.path.join(logger.get_dir(), f'model{step:06d}.pt'))
...
def create_argparser():
    defaults = dict(data_dir='', val_data_dir='', noised=True, iterations=150000, lr=0.0003, weight_decay=0.0, anneal_lr=False, batch_size=4, microbatch=(- 1), schedule_sampler='uniform', resume_checkpoint='', log_interval=10, eval_interval=5, save_interval=10000)
