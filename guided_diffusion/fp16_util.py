...
def make_master_params(param_groups_and_shapes):
    ...
    for (param_group, shape) in param_groups_and_shapes:
        master_param = jt.array(jt.flatten([param.detach().float() for (_, param) in param_group]).view(shape))
...
def model_grads_to_master_grads(param_groups_and_shapes, master_params):
    for (master_param, (param_group, shape)) in zip(master_params, param_groups_and_shapes):
        master_param.grad = jt.flatten([param_grad_or_zeros(param) for (_, param) in param_group]).view(shape)
...
class MixedPrecisionTrainer():
    ...
    def _optimize_normal(self, opt: jt.optim.Optimizer):
        # (grad_norm, param_norm) = self._compute_norms(opt=opt)
        # logger.logkv_mean('grad_norm', grad_norm)
        # logger.logkv_mean('param_norm', param_norm)
        opt.step()
        return True
    ...
    def _compute_norms(self, opt: jt.optim.Optimizer, grad_scale=1.0):
        ...
        for p in self.master_params:
            with jt.no_grad():
                param_norm += (jt.norm(p, p=2) ** 2)
                if (p.opt_grad(opt) is not None):
                    grad_norm += (jt.norm(p.opt_grad(opt), p=2) ** 2)
